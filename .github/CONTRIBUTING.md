...
License that covers this project. The plugin imports community-contributed
data from the Garmin ActiveCaptain API and from OpenStreetMap via the
Overpass API; keep the "Data sourced from Garmin Active Captain"
attribution on every ActiveCaptain-rendered note and the
"© OpenStreetMap contributors (ODbL)" attribution on every
OpenSeaMap-rendered note. The OpenStreetMap data is published under the
